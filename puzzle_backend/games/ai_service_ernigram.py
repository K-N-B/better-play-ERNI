...
def fetch_raw_csv_data(file_path="ERNI_Content.csv", text_column_index=0):
    ...
    try:
        ...
        print(f"⚠️ Error: CSV row is too short. It does not have data at column index {text_column_index}.")
...
class ErnigramGeneratorAI:
    ...
    def generate_from_raw_text(self, raw_text_list, used_phrases, dominant_theme=None): # <--- ACCEPT HISTORY
            ...
            **AVAILABLE TEXT BLOCKS:**
            ...
            try:
                response = self.client.chat.completions.create(
                    ...
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "You are a puzzle generator. Your only output is the required JSON object."},
                        {"role": "user", "content": prompt},
                    ],
                    temperature=1.2,
                    max_tokens=350,
