...
class LeaderboardAggregator:
    ...
    def update_daily_scores(submission_date: date):
        ...
        user_scores = submissions.values('user').annotate(total_score=Sum('points_awarded'))
        ...
        for user_score in user_scores:
            ...
            DailyIndividualScore.objects.update_or_create(
                user_id=user_id, date=submission_date, defaults={'score': score}
            )
    ...
    def update_weekly_scores(week_start: date):
        ...
        daily_scores = DailyIndividualScore.objects.filter(date__gte=week_start, date__lte=week_end)

        user_weekly_scores = daily_scores.values('user').annotate(total_score=Sum('score'))
    ...
    def update_monthly_scores(month_start: date):
        ...
        print(
            f"[LeaderboardAggregator] Updating monthly scores for {month_start} to {month_end}..."
        )
        ...
        user_monthly_scores = daily_scores.values('user').annotate(total_score=Sum('score'))
