...
class CreateChallengeSerializer(serializers.Serializer):
    ...
    def validate_recipient_id(self, value):
        ...
        if request:
            ...
            if current_user and current_user.id == value:
                print("[CreateChallengeSerializer] User trying to challenge themselves")  # Debug
                raise serializers.ValidationError("You cannot challenge yourself.")
        else:
            print("[CreateChallengeSerializer] No request in context!")  # Debug
    ...
    def validate_submission_id(self, value):
        ...
        try:
            ...
            print(
                f"[CreateChallengeSerializer] Found submission: {submission.id}, owner: {submission.user.username}"
            )  # Debug
        ...
        if request:
            current_user = getattr(request, 'user', None)
            print(
                f"[CreateChallengeSerializer] Current user: {current_user}, Submission owner: {submission.user}"
            )  # Debug

            if current_user and submission.user != current_user:
                print("[CreateChallengeSerializer] Submission doesn't belong to user")  # Debug
                raise serializers.ValidationError(
                    "You can only challenge with your own submissions."
                )
        else:
            print(
                "[CreateChallengeSerializer] No request in context for submission validation!"
            )  # Debug
    ...
    def validate(self, data):
        ...
        if existing_challenge:
            print(
                f"[CreateChallengeSerializer] Challenge already exists for submission {submission_id}"
            )  # Debug
            raise serializers.ValidationError(
                "A challenge has already been created for this submission."
            )

        print("[CreateChallengeSerializer] Validation passed!")  # Debug
        return data
