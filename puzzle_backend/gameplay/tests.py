...
class SaveProgressViewTests(TestCase):
    def setUp(self):
        ...
        self.sudoku_content_type = ContentType.objects.get_for_model(
            self.placeholder_sudoku
        )
        self.ernigram_content_type = ContentType.objects.get_for_model(
            self.placeholder_ernigram
        )
    ...
    def test_illegal_guess_rejected_on_hard_mode(self):
        ...
        self.assertIn(
            "Maximum of 6 guesses for 'HARD' difficulty exceeded.",
            response.json()["error"],
        )
    ...
    def test_limit_not_exceeded_on_easy_mode(self):
        ...
        payload = {
            "progress_data": {
                "guesses": ["RATES", "TRAIN", "FIGHT", "LIGHT", "BLANK", "SOLVE"]
            },
            "time_spent_ms": 200000,
            "difficulty": "EASY",
        }
    ...
    def test_submission_rejected_due_to_time_limit(self):
        ...
        self.assertIn(
            "Time limit of 5 minutes for 'EASY' difficulty exceeded.",
            response.json()["error"],
        )
    ...
    def test_final_submission_rejected_by_time(self):
        ...
        self.assertEqual(response.status_code, 403)  # Forbidden
        self.assertIn(
            "Time limit of 5 minutes for 'EASY' difficulty was exceeded.",
            response.json()["error"],
        )
        ...
        self.assertEqual(Submission.objects.count(), 0)
        self.assertTrue(
            PuzzleAttempt.objects.filter(pk=time_violating_attempt.pk).exists()
        )
    ...
    def test_sudoku_save_rejected_by_hint_limit(self):
        ...
        url = self._get_save_url(
            self.daily_puzzle_easy, self.placeholder_sudoku
        )  # EASY limit is 5 hints
        ...
        self.assertIn(
            "Maximum of 5 hints for 'EASY' difficulty exceeded.",
            response.json()["error"],
        )
    ...
    def test_sudoku_submission_rejected_by_time_limit(self):
        ...
        self.assertEqual(response.status_code, 403)
        self.assertIn(
            "Time limit of 15 minutes for 'EASY' difficulty was exceeded.",
            response.json()["error"],
        )
        self.assertTrue(
            PuzzleAttempt.objects.filter(pk=time_violating_attempt.pk).exists()
        )  # Attempt should NOT be deleted
    ...
    def test_sudoku_submission_rejected_if_unsolved(self):
        ...
        self.assertTrue(
            PuzzleAttempt.objects.filter(pk=attempt.pk).exists()
        )  # Attempt should NOT be deleted
    ...
    def test_ernigram_save_rejected_by_mistake_limit(self):
        ...
        self.assertEqual(response.status_code, 403)
        self.assertIn(
            "Maximum of 4 mistakes for 'HARD' difficulty exceeded.",
            response.json()["error"],
        )
    ...
    def test_get_progress_retrieves_existing_attempt(self):
        ...
        self.assertEqual(
            data["progress_data"]["guesses"],
            self.solved_attempt.progress_data["guesses"],
        )
