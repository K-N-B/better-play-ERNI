...
def auth_callback(request):
    ...
    if error:
        ...
        return redirect(
            f"{frontend_login_url}?error={error}&desc={urllib.parse.quote(error_description)}"
        )
    ...
    try:
        ...
        if "error" in result:
            print(
                f"MSAL Token Acquisition Error: {result.get('error')} - {result.get('error_description')}"
            )
            return redirect(
                f"{frontend_login_url}?error=token_failed&desc={result.get('error_description', '')}"
            )
        ...
        graph_response = requests.get(
            graph_url, headers={"Authorization": f"Bearer {access_token}"}, timeout=10
        )
        ...
        if not email:
            ...
            print(
                f"Warning: No email found for Azure user {azure_object_id}. Using placeholder: {email}"
            )
        ...
        print(
            f"User '{user.username}' (AzureID: {azure_object_id}) logged in successfully via Azure AD. Created: {created}."
        )
