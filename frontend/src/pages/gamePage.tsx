...
const hasResumableProgress = (
  ...
  switch (gameType) {
    case "wordle": {
      ...
      return (
        (wordleProgress?.guesses?.length ?? 0) > 0 &&
        !wordleProgress?.isGameOver
      );
    }
...
export const GamePage = () => {
  ...
  const difficultyFromUrl = searchParams
    .get("difficulty")
    ?.toLowerCase() as Difficulty | null;
  ...
  if (foundSubmission) {
    content = (
      <AlreadyPlayedScreen
        ...
        difficulty={
          (foundSubmission.difficulty as Difficulty) || activeDifficulty
        }
      />
    );
  }
          ...
          {challengeId && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4 text-center">
              <p className="text-blue-800 font-medium">
                🎯 Challenge Mode - Playing on{" "}
                <span className="uppercase font-bold">{activeDifficulty}</span>{" "}
                difficulty
              </p>
