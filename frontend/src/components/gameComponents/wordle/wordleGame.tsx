...
import {
  ...
  getSavedAttempt,
  saveProgress,
} from "../../../api/gameService"; // Adjust path if needed
...
export const WordleGame = ({
  ...
  useEffect(() => {
    ...
    guesses,
    currentRow,
    /*letterStatuses,*/ isGameOver,
    time,
    loading,
    puzzle.id,
  ]); // Removed letterStatuses & gameResult
  ...
  const endGame = async (tries: number, won: boolean) => {
    ...
    try {
      ...
      if (challengeId && submissionIdForResultModal) {
        console.log(
          `[WordleGame] Completing challenge ${challengeId} with submission ${submissionIdForResultModal}`
        );
        await completeChallenge(challengeId, {
          submission_id: submissionIdForResultModal,
        });
        console.log(
          `[WordleGame] Challenge ${challengeId} marked as complete.`
        );
      } else if (challengeId) {
        console.error(
          "[WordleGame] Challenge ID present but failed to get submission ID."
        );
      }
    } catch (err) {
      console.error(
        "Error during game end submission/challenge completion:",
        err
      );
    } finally {
      setGameResult({
