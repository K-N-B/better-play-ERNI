...
export const GameCard: React.FC<GameCardProps> = ({
  ...
  return (
    <Link
      to={path}
      className={` rounded-2xl p-8 sm:p-6 h-full w-full transition-transform duration-100 ease-in-out overflow-hidden
    ...
    >
      <div className="flex flex-col items-center justify-center text-center text-white">
        <div className="flex flex-row items-center justify-center gap-2 mb-2">
          <IconComponent className="size-10 sm:size-4" strokeWidth={2} />{' '}
          {/* Adjust size/stroke */}
          <div className="text-xl sm:text-xl font-semibold">{title}</div>
        </div>

        <div className="text-sm sm:text-sm leading-5 sm:leading-4">
          {subtitle}
        </div>
      </div>
    </Link>
