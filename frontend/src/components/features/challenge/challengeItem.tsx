...
export const ChallengeItem: React.FC<ChallengeItemProps> = ({
  ...
  const handlePlayClick = async (e: React.MouseEvent) => {
    ...
    console.log(
      '[ChallengeItem] challenge.puzzle_type:',
      challenge.puzzle_type,
    );
    console.log(
      '[ChallengeItem] URL will be:',
      `/game/${challenge.puzzle_type}?challenge_id=${challenge.id}`,
    );
  ...
  return (
    ...
    >
      ...
      <div className="flex-grow">
        <p className="text-sm">
          Challenge {isPending ? 'from' : 'vs'}{' '}
          <strong className="font-medium">{opponent.username}</strong> on{' '}
        ...
        {challenge.recipient_submission && (
          <p className="text-xs text-black">
            ...
            {won && (
              <span className="ml-2 inline-block px-2 py-0.5 rounded bg-green-200 text-green-800 text-[10px] font-semibold">
                You Won!
              </span>
            )}
            {lost && (
              <span className="ml-2 inline-block px-2 py-0.5 rounded bg-red-200 text-red-800 text-[10px] font-semibold">
                You Lost
              </span>
            )}
            {!isPending && !won && !lost && (
              <span className="ml-2 inline-block px-2 py-0.5 rounded bg-gray-100 text-gray-700 text-[10px] font-semibold">
                Tie
              </span>
            )}
          </p>
