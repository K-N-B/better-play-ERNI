...
const PodiumItem: React.FC<{
  ...
  if (type === "department") {
    ...
    if ("user" in entry && entry.user) {
      ...
      console.log(
        `[PodiumItem] Extracted username: ${name}, Profile URL: ${profileImageUrl}`
      );
    } else {
  ...
  console.log(
    `[PodiumItem] Final display - Rank: ${rank}, Name: ${name}, Score: ${score}`
  );

  return (
    <div
      className={`col-span-1 flex justify-center ${rank === 1 ? "mt-0" : "mt-4"}`}
    >
      <div
        id={`flag-${rank}`}
        className={`relative flex justify-center ${sizeClass}`}
      >
        {/* Base SVG */}
        <svg
        ...
        <div
          className={`absolute z-20 rounded-full w-20 h-20 overflow-hidden ${topClass}`}
        >
          {profileImageUrl ? (
            <img
              src={profileImageUrl}
        ...
        {/* Text */}
        <div
          className={`absolute z-20 ${rank === 1 ? "top-32" : "top-28"} text-center w-full px-2`}
        >
          <p
            className={`text-sm font-semibold m-0 p-0 leading-tight ${textColor} truncate`}
          >
            {name}
          </p>
          <p className={`text-xs m-0 p-0 leading-tight ${textColor}`}>
            {score} pts
          </p>
        </div>
...
export const LeaderboardPodium: React.FC<LeaderboardPodiumProps> = ({
  ...
  console.log(
    `[LeaderboardPodium] Received topThree array, length: ${topThree.length}, type: ${type}`
  );
  ...
  return (
    <div className="flex flex-row justify-center items-end gap-4 md:gap-8 pb-4">
      {podiumConfig.map((config) => {
        ...
        console.log(
          `[LeaderboardPodium] Rendering config for rank ${config.rank}, has entry:`,
          !!entry
        );
        ...
        return entry ? (
          ...
          <div
            key={`placeholder-${config.rank}`}
            className={`col-span-1 ${config.size}`}
          >
