...
export const RewardCard: React.FC<RewardCardProps> = ({
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      setMessage(
        err.message || "Failed to claim reward due to an unexpected error."
      );
  ...
  return (
    <div
      className={clsx(
        "bg-white rounded-2xl shadow-md overflow-hidden flex flex-col transition-shadow hover:shadow-lg",
    ...
    >
      ...
      {reward.image ? (
        <img
          ...
          onError={(e) => {
            e.currentTarget.src =
              "https://placehold.co/300x200/e2e8f0/94a3b8?text=Image+Unavailable";
      ...
      <div className="p-4 flex flex-col flex-grow">
          ...
          <button
            ...
            className={clsx(
              ...
              canAfford &&
                ...
                claimStatus !== "success" &&
                !isClaimLimitReached
                ? "bg-primary text-white border-primary-dark hover:bg-primary-dark active:translate-y-px"
                : "",
              isClaiming
                ? "bg-gray-400 text-white border-gray-500 cursor-wait"
                : "",
              claimStatus === "success"
        ...
        {message && (
          <p
            className={`text-xs mt-2 text-center font-medium h-4 ${claimStatus === "success" ? "text-green-600" : "text-red-600"}`}
          >
            {message}
          </p>
        )}
