...
function App() {
  ...
  const initializePuzzle = useCallback((mode, solutionWord, persisted = {}) => {
    ...
    const puzzleCompleted = Boolean(
      persisted.completed || persisted.isGameOver
    );
    ...
    const storedTimer =
      typeof persisted.timer === "number" ? persisted.timer : 0;
    setTimer(storedTimer);
    const resumeTimer =
      !puzzleCompleted &&
      (persisted.isTimerRunning === undefined
        ? true
        : Boolean(persisted.isTimerRunning));
  ...
  const handleHint = () => {
    ...
    setHintText(
      `Hint: Position ${
        indexToReveal + 1
      } contains the letter ${revealedLetter}.`
    );
  ...
  const forfeitPuzzle = useCallback(
    ({ showAlert = true, message } = {}) => {
      ...
      if (showAlert) {
        setTimeout(
          () => alert(`Puzzle forfeited. The solution was ${solution}.`),
          100
        );
      }
    },
      ...
      {!difficulty ? (
        <>
          <div className="difficulty-status-wrapper">
            {isFetchingWords && (
              <p className="difficulty-status">
                Loading word bank from Datamuse...
              </p>
            )}
            {wordFetchError && (
        ...
        */}
          <div>
            <div className="bg-[#9ED7BF]  flex justify-evenly items-center rounded-xl">
              <div className="bg-white p-20 rounded-xl m-6">
                <Grid
                  guesses={guesses}
              ...
              <div className="mode-banner flex flex-col text-center gap-6">
                <div>
                ...
                */}
                ...
                <div className="mode-actions flex flex-col gap-2">
                  <button
                    type="button"
